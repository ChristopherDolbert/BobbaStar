...
if (isset($_POST['texte'])) {
	...
	if ($texte != "" && $sujet != "") {
		...
		$insertn2->bindValue(':categorie', 'Contact');
...
<body>
	...
	<div style="display:block; position:fixed;" title="Quitter le service client" onmouseover="tooltip.show(this)" onmouseout="tooltip.hide(this)">
		<a href="<?PHP echo $url; ?>/moi.php"><img src="<?PHP echo $url; ?>/service_client/img/retour.gif" style="margin-left:10px;" /></a>
	</div>
	<div id="container">
		<div id="haut">Contact nous !</div>
		<div id="mil">
			<center><?PHP if (isset($affichage)) {
						echo "<br>" . $affichage . "";
					} ?></center><br>
			Salut à toi <?PHP echo $user['username']; ?> ! Tu es sur la catégorie <span style="color: #0066CC"><b>Contact nous</b></span> du HelpTool de <?PHP echo $sitename; ?>. Si tu ne voulais pas aller là, clique sur le bouton "Revenir à l'accueil" en bas de la page.<br><br>

			Voici le formulaire :
			<p>
			<form method="post" action="#">
				<span style="color: #0099FF;">Votre pseudo sur <?PHP echo $sitename; ?> </span> <input type="text" value="<?PHP echo $user['username']; ?>" name="username" readonly="readonly" /><br /><br />
				<span style="color: #0099FF;">Votre e-mail : </span><input type="text" value="<?PHP echo $user['mail']; ?>" name="mail" readonly="readonly" /><br /><br />
				<span style="color: #0099FF;">Qu'est-ce qui se passe ? </span><input type="text" name="sujet" style="width: 360px;" maxlength="250" value="<?php if (!empty($_POST["sujet"])) {
