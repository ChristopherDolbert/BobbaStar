...
$sql = $bdd->prepare("SELECT noob,gift,sort,roomid,lastgift FROM users WHERE id=:my_id LIMIT 1");
...
if ($row['gift'] < 3) {
	if ($row['noob'] == 1) {
		if ($row['lastgift'] < date("d-m-Y")) {
			$insert = $bdd->prepare("INSERT INTO cms_noobgifts (userid,gift,read) VALUES (:my_id,:gift,'0')");
			$insert->execute(array(':my_id' => $user['id'], ':gift' => $row['gift']));

			$update = $bdd->prepare("UPDATE users SET lastgift=:lastgift,gift=gift+1 WHERE id=:my_id LIMIT 1");
			$update->execute(array(':lastgift' => date("d-m-Y"), ':my_id' => $user['id']));
		}
	}
}
...
<div class="gift-content-container">
	<?php if ($row['gift'] < 3) { ?>
		<p class="gift-content">
			Your next piece of free furniture will be <strong><?php if ($row['gift'] == 0) {
																	echo "My first Habbo stool";
																} elseif ($row['gift'] == 1) {
																	echo "plant";
																} ?></strong>
		</p>
...
?>
	...
	<ul>
		<li>1. Buy some credits from the <a href="/credits">credits</a> section</li>
	...
	<script type="text/javascript">
		...
		<?php $bdd->query("UPDATE users SET noob='0'");
		?>
	</script>
<?php } ?>