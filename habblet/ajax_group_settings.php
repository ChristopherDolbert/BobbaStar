...
<form action="#" method="post" id="group-settings-form">
    ...
    <div id="group-settings">
        ...
        <div id="group-settings-type" class="group-settings-pane group-settings-selection">
            ...
            <input type="radio" name="group_type" id="group_type" value="0" <?php if ($groupdata['type'] == "0") {
                                                                                echo "checked=\"checked\"";
                                                                            } elseif ($groupdata['type'] == "3") {
            ...
            <input type="radio" name="group_type" id="group_type" value="1" <?php if ($groupdata['type'] == "1") {
                                                                                echo "checked=\"checked\"";
                                                                            } elseif ($groupdata['type'] == "3") {
                                                                                echo "disabled=\"disabled\"";
                                                                            } ?> />
            ...
            <input type="radio" name="group_type" id="group_type" value="2" <?php if ($groupdata['type'] == "2") {
                                                                                echo "checked=\"checked\"";
                                                                            } elseif ($groupdata['type'] == "3") {
                                                                                echo "disabled=\"disabled\"";
                                                                            } ?> />
            ...
            <input type="radio" name="group_type" id="group_type" value="3" <?php if ($groupdata['type'] == "3") {
    ...
    <div id="forum-settings" style="display: none;">
        <div id="forum-settings-type" class="group-settings-pane group-settings-selection">
            ...
            <input type="radio" name="forum_type" id="forum_type" value="0" <?php if ($groupdata['pane'] == 0) {
                                                                                echo 'checked="checked"';
                                                                            } ?> />
            <div class="description">
                Public<br />
            ...
            <input type="radio" name="forum_type" id="forum_type" value="1" <?php if ($groupdata['pane'] == 1) {
                                                                                echo 'checked="checked"';
                                                                            } ?> />
            <div class="description">
                Private<br />
        ...
        <div id="forum-settings-topics" class="group-settings-pane group-settings-selection">
            ...
            <input type="radio" name="new_topic_permission" id="new_topic_permission" value="2" <?php if ($groupdata['topics'] == 2) {
                                                                                                    echo 'checked="checked"';
                                                                                                } ?> />
            <div class="description">
                Admin<br />
            ...
            <input type="radio" name="new_topic_permission" id="new_topic_permission" value="1" <?php if ($groupdata['topics'] == 1) {
                                                                                                    echo 'checked="checked"';
                                                                                                } ?> />
            <div class="description">
                Members<br />
            ...
            <input type="radio" name="new_topic_permission" id="new_topic_permission" value="0" <?php if ($groupdata['topics'] == 0) {
                                                                                                    echo 'checked="checked"';
                                                                                                } ?> />
            <div class="description">
                Everyone<br />
    ...
    <div id="room-settings" style="display: none;">
        <?php
        $sql = $bdd->prepare("SELECT id,name,description FROM rooms WHERE owner=:rawname");
        $sql->execute(['rawname' => $rawname]);
        $row = $sql->fetch(PDO::FETCH_ASSOC);

        $groupdetails = $bdd->prepare("SELECT * FROM guilds WHERE room_id=:roomId AND id=:groupId LIMIT 1");
        $groupdetails->execute(['roomId' => $row['id'], 'groupId' => $groupid]);
        ...
        <div id="room-settings-id" class="group-settings-pane-wide group-settings-selection">
            <ul>
                <li><input type="radio" name="roomId" value=" " <?php if ($group['roomid'] == 0 or $group['roomid'] == "" or $group['roomid'] == " ") {
                                                                    echo "CHECKED";
                                                                } ?> />
                    <div>No room</div>
                </li>
                ...
                ?>
                    <li class="<?php echo $even; ?>">
                        <input type="radio" name="roomId" value="<?php echo $row['id']; ?>" <?php if ($group['roomid'] == $row['id']) {
                                                                                                echo "CHECKED";
                                                                                            } ?> /><a href="client.php?forwardId=2&roomId=<?php echo $row['id']; ?>" onclick="HabboClient.roomForward(this, '<?php echo $row['id']; ?>', 'private'); return false;" target="client" class="room-enter">Go to room!</a>
                        <div>
                            <?php echo HoloText($row['name']);
                            if ($row['name'] == "") { ?>&nbsp;<?php } ?><br />
                            <span class="room-description"><?php echo HoloText($row['description']);
                                                            if ($row['description'] == "") { ?>&nbsp;<?php } ?></span>
                        </div>
                    </li>
                <?php } ?>
            </ul>
