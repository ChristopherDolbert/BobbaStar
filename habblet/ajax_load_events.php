...
<ul class="habblet-list">
	<?php
	$getem = $bdd->prepare("SELECT * FROM events WHERE category = :category");
	$getem->execute(array(':category' => $category));
	$i = 0;

	while ($row = $getem->fetch(PDO::FETCH_ASSOC)) {
		$roomget = $bdd->prepare("SELECT * FROM rooms WHERE id = :roomid");
		$roomget->execute(array(':roomid' => $row['roomid']));
		$roomrow = $roomget->fetch(PDO::FETCH_ASSOC);

		$userget = $bdd->prepare("SELECT * FROM users WHERE id = :userid");
		$userget->execute(array(':userid' => $row['userid']));
		$userrow = $userget->fetch(PDO::FETCH_ASSOC);
		...
		if ($roomrow['incnt_max'] == 0) {
			$roomrow['incnt_max'] = 1;
		}
		$data[$i] = ($roomrow['incnt_now'] / $roomrow['incnt_max']) * 100;

		// Base room icon based on this - percentage levels may not be Habbo-like
