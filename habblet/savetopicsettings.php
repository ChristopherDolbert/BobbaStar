...
if(is_numeric($topicid)){
	if($user_rank > 5){
		$check = mysqli_query($con,("SELECT title, type FROM cms_forum_threads WHERE id = '".$topicid."' LIMIT 1");
		$exists = mysql_num_rows($check);
		if($exists > 0){
			mysqli_query($con,("UPDATE cms_forum_threads SET type = '".$type."', title = '".$topicname."' WHERE id = '".$topicid."' LIMIT 1") or die(mysql_error());
