...
if(!empty($topicId) && is_numeric($topicId)){
	$check = mysqli_query($con,("SELECT * FROM cms_forum_threads WHERE id = '".$topicId."' LIMIT 1") or die(mysql_error());
...
if($valid_thread == true){
	mysqli_query($con,("INSERT INTO cms_forum_posts (threadid,message,author,date) VALUES ('".$topicId."','".$message."','".$name."','".$date_full."')");
	mysqli_query($con,("UPDATE users SET postcount = postcount + 1 WHERE id = '".$user['id']."' LIMIT 1") or die(mysql_error());
	mysqli_query($con,("UPDATE cms_forum_threads SET lastpost_author = '".$name."', lastpost_date = '".$date_full."', unix = '".strtotime('now')."', posts = posts + 1 WHERE id = '".$topicId."' LIMIT 1") or die(mysql_error());
}
...
$get_em = mysqli_query($con,("SELECT * FROM cms_forum_posts WHERE threadid = '".$topicId."' ORDER BY id ASC LIMIT ".$query_min.", 10") or die(mysql_error());
$dynamic_id = 0;
...
while($row = mysql_fetch_assoc($get_em)){
	...
	$userquery = mysqli_query($con,("SELECT * FROM users WHERE name = '".$row['author']."' LIMIT 1");
	...
	<td class=\"post-list-message\" valign=\"top\" colspan=\"2\">
			...
			if($user_rank > 5 || $user['id'] == $userdata['id']){
