...
$tag_1 = filter_var($_POST['tag1'], FILTER_SANITIZE_STRING);
$tmp = $bdd->prepare("SELECT id FROM cms_tags WHERE tag = :tag");
$tmp->execute(['tag' => $tag_1]);
$tag_1_results = $tmp->rowCount();

$tag_2 = filter_var($_POST['tag2'], FILTER_SANITIZE_STRING);
$tmp = $bdd->prepare("SELECT id FROM cms_tags WHERE tag = :tag");
$tmp->execute(['tag' => $tag_2]);
$tag_2_results = $tmp->rowCount();

if (empty($tag_1) || empty($tag_2)) {
    exit;
}
...
echo "<br />
			    ".htmlspecialchars($tag_1)."
			(".$tag_1_results.") hits
            <br/>
                ".htmlspecialchars($tag_2)."
            (".$tag_2_results.") hits 
		    </div>
