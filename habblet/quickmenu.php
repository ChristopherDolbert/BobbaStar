...
switch($mode){
	case 1:
		$str = "friends";
		$get_em = mysqli_query($con,("SELECT friendid,userid FROM messenger_friendships WHERE userid = '".$user['id']."' OR friendid = '".$user['id']."' ORDER BY friendid LIMIT 500") or die(mysql_error());
		break;
	case 2:
		$str = "groups";
	...
	case 3:
		$str = "rooms";
		$get_em = mysqli_query($con,("SELECT * FROM rooms WHERE owner  = '".$name."' ORDER BY name ASC LIMIT 100") or die(mysql_error());
		break;
}
	...
	if($results > 0){
		...
		echo "<ul id=\"offline-friends\">\n";
			...
			if($row['userid'] == $user['id']){
				$userdatasql = mysqli_query($con,("SELECT name FROM users WHERE id = '".$row['friendid']."' LIMIT 1") or die(mysql_error());
			} else {
				$userdatasql = mysqli_query($con,("SELECT name FROM users WHERE id = '".$row['userid']."' LIMIT 1") or die(mysql_error());
			}
		...
		$num = 0;
			...
			while($row = mysql_fetch_assoc($get_em)){
				...
				$check = mysqli_query($con,("SELECT name,ownerid FROM groups_details WHERE id = '".$group_id."' LIMIT 1") or die(mysql_error());
				...
				if($row['member_rank'] > 1 && $groupdata['ownerid'] !== $user['id']){ echo "<div class=\"admin-group\" title=\"Admin\"></div>\n"; }
				if($groupdata['ownerid'] == $user['id'] && $row['member_rank'] > 1){ echo "<div class=\"owned-group\" title=\"Owner\"></div>\n"; }
