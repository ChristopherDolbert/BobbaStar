...
$my_id = $user['id']; // Assuming you have stored the user ID in a session variable
...
function UpdateOrInsert($type, $amount, $data, $my_id)
{
	...
	$stmt = $bdd->prepare("SELECT id FROM cms_homes_inventory WHERE data = :data AND userid = :userid AND type = :type LIMIT 1");
	$stmt->bindParam(':data', $data);
	$stmt->bindParam(':userid', $my_id);
	...
	if ($exists > 0) {
		$stmt = $bdd->prepare("UPDATE cms_homes_inventory SET amount = amount + :amount WHERE userid = :userid AND type = :type AND data = :data LIMIT 1");
		...
		$stmt = $bdd->prepare("INSERT INTO cms_homes_inventory (userid, type, subtype, data, amount) VALUES (:userid, :type, '0', :data, :amount)");
		$stmt->bindParam(':userid', $my_id);
		$stmt->bindParam(':type', $type);
		$stmt->bindParam(':data', $data);
		$stmt->bindParam(':amount', $amount);
...
function UpdateOrDelete($id, $my_id)
{
	...
	$stmt = $bdd->prepare("SELECT amount FROM cms_homes_inventory WHERE id = :id AND userid = :userid LIMIT 1");
	...
	if ($exists > 0) {
		...
		if ($row['amount'] > 1) {
			$stmt = $bdd->prepare("UPDATE cms_homes_inventory SET amount = amount - 1 WHERE id = :id AND userid = :userid LIMIT 1");
			$stmt->bindParam(':id', $id);
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
		} else {
			$stmt = $bdd->prepare("DELETE FROM cms_homes_inventory WHERE id = :id AND userid = :userid LIMIT 1");
			$stmt->bindParam(':id', $id);
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
		}
...
if ($mode == "inventory") {
	...
	$stmt = $bdd->prepare("SELECT data FROM cms_homes_inventory WHERE type = '1' AND userid = :userid LIMIT 1");
	$stmt->bindParam(':userid', $my_id);
	$stmt->execute();
	...
	if ($valid > 0) {
		...
		$jsonArray = [
			["Inventory", "Webstore"],
			[
				formatThing(1, $row['data'], true),
				formatThing(1, $row['data'], false),
				$row['data'],
				"Sticker",
				null,
				1
						...
						<ul class="purchase-subcategory-list" id="main-category-items-1">
							<?php if ($user['rank'] > 5) { ?>
								<li id="subcategory-1-50-stickers" class="subcategory">
									<div><strong>
											<font color='red'><?php echo $shortname; ?> Staff
										</strong></font>
									</div>
								</li>
							<?php } ?>
			...
			<div id="inventory-items-container">
				...
				<div id="inventory-items">
					<ul id="inventory-item-list">
						<?php
						$stmt = $bdd->prepare("SELECT * FROM cms_homes_inventory WHERE userid = :userid AND type = '1'");
						$stmt->bindParam(':userid', $my_id);
						$stmt->execute();

						$get_em = $stmt->fetchAll(PDO::FETCH_ASSOC);
						$number = count($get_em);
...
<p>To buy stickers, backgrounds and notes, click on the Webstore tab and browse through the categories. When you find something you like, click 'Purchase' to buy it.</p>
	...
	if ($type == "stickers") {
		...
		$stmt = $bdd->prepare("SELECT * FROM cms_homes_inventory WHERE userid = :userid AND type = '3'");
		$stmt->bindParam(':userid', $my_id);
		$stmt->execute();
		$typ = "note";
		...
		$stmt = $bdd->prepare("SELECT * FROM cms_homes_inventory WHERE userid = :userid AND type = '4'");
		$stmt->bindParam(':userid', $my_id);
		$stmt->execute();
		$typ = "background";
	} else {
	...
	$get_em = $stmt; // Assuming $stmt is the PDOStatement object

	if ($typ !== "widget") {
		$number = $get_em->rowCount();
		$results = $get_em->fetchAll(PDO::FETCH_ASSOC);
...
<p>To buy stickers, backgrounds and notes, click on the Webstore tab and browse through the categories. When you find something you like, click 'Purchase' to buy it.</p>
	...
	<div class=\"webstore-frank-image\"><img src=\"./web-gallery/images/frank/sorry.gif\" alt=\"\" width=\"57\" height=\"88\" /></div>
		...
		if ($linked > 0) { // Group Mode
			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE groupid = :groupid AND type = '2' AND subtype = '3' LIMIT 1");
			$stmt->bindParam(':groupid', $groupid);
			$stmt->execute();
			$placed_memberwidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE groupid = :groupid AND type = '2' AND subtype = '4' LIMIT 1");
			$stmt->bindParam(':groupid', $groupid);
			$stmt->execute();
			$placed_guestbookwidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE groupid = :groupid AND type = '2' AND subtype = '5' LIMIT 1");
			$stmt->bindParam(':groupid', $groupid);
			$stmt->execute();
			$placed_traxwidget = $stmt->rowCount();


			echo "<ul id=\"inventory-item-list\">";
			echo "<li id=\"inventory-item-p-3\"
		title=\"My Groups\" class=\"webstore-widget-item";
			if ($placed_memberwidget > 0) {
		...
		title=\"Guestbook\" class=\"webstore-widget-item";
			if ($placed_guestbookwidget > 0) {
				echo " webstore-widget-disabled";
			}
			echo "\">
	...
	<li id=\"inventory-item-p-5\"
		title=\"Traxplayer\" class=\"webstore-widget-item";
			if ($placed_traxwidget > 0) {
				echo " webstore-widget-disabled";
			}
			echo "\">
		...
		<div class=\"webstore-widget-description\">
			...
			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type = '2' AND subtype = '2' LIMIT 1");
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
			$placed_groupwidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type = '2' AND subtype = '3' LIMIT 1");
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
			$placed_roomwidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type = '2' AND subtype = '4' LIMIT 1");
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
			$placed_guestbookwidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type = '2' AND subtype = '5' LIMIT 1");
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
			$placed_friendswidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type = '2' AND subtype = '6' LIMIT 1");
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
			$placed_traxwidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type = '2' AND subtype = '7' LIMIT 1");
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
			$placed_scoreswidget = $stmt->rowCount();

			$stmt = $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type = '2' AND subtype = '8' LIMIT 1");
			$stmt->bindParam(':userid', $my_id);
			$stmt->execute();
			$placed_badgeswidget = $stmt->rowCount();


			echo "<ul id=\"inventory-item-list\">";
			echo "<li id=\"inventory-item-p-7\"
		title=\"High scores widget\" class=\"webstore-widget-item";
			if ($placed_scoreswidget > 0) {
				echo " webstore-widget-disabled";
			}
			echo "\">
	...
	<li id=\"inventory-item-p-3\"
		title=\"Rooms Widget\" class=\"webstore-widget-item";
			if ($placed_roomwidget > 0) {
				echo " webstore-widget-disabled";
			}
			echo "\">
	...
	<li id=\"inventory-item-p-5\"
		title=\"My Friends\" class=\"webstore-widget-item";
			if ($placed_friendswidget > 0) {
				echo " webstore-widget-disabled";
			}
			echo "\">
	...
	<li id=\"inventory-item-p-8\"
		title=\"My Badges\" class=\"webstore-widget-item";
			if ($placed_badgeswidget > 0) {
				echo " webstore-widget-disabled";
			}
			echo "\">
	...
	$stmt = $bdd->prepare("SELECT * FROM cms_homes_catalouge WHERE category = '19' ORDER BY id ASC LIMIT 1");
	$stmt->execute();
	$valid = $stmt->rowCount();
			...
			<div id="webstore-content-container">
				<div id="webstore-items-container">
					...
					<div id="webstore-items">
						...
						$stmt = $bdd->prepare("SELECT * FROM cms_homes_catalouge WHERE category = :category");
...
<p>Watch this space - we add new items all the time!</p>
			...
			<div class=\"webstore-item-mask\">
				...
				<div id="webstore-preview-container">
					...
					<div id="webstore-preview"><?php
												$stmt = $bdd->prepare("SELECT * FROM cms_homes_catalouge WHERE id = '1' AND category = '19' LIMIT 1");
												$stmt->execute();
						...
						<div id="webstore-preview-purse">
							You have:<br /><b><?php echo $user['credits']; ?> Credits</b><br />
							<?php if ($user['credits'] < $row['price']) { ?><span class="webstore-preview-error">You don't have enough Credits to buy this item.</span><br />
								<a href="credits.php" target=_blank>Get Credits</a><?php } ?>
						</div>

						<div id="webstore-preview-purchase" class="clearfix">
							<div class="clearfix">
								<a href="#" class="new-button <?php if ($user['credits'] < $row['price']) { ?>disabled-button<?php } ?>" <?php if ($user['credits'] < $row['price']) { ?>disabled="disabled" <?php } ?> id="webstore-purchase<?php if ($user['credits'] < $row['price']) { ?>-disabled<?php } ?>"><b>Buy</b><i></i></a>
							</div>
						</div>
	...
	$stmt =  $bdd->prepare("SELECT * FROM cms_homes_catalouge WHERE id = :productId AND category = :subCategoryId LIMIT 1");
	...
	?>
		...
		$stmt =  $bdd->prepare("SELECT * FROM cms_homes_catalouge WHERE id = :productId LIMIT 1");
		$stmt->bindParam(':productId', $productId);
		$stmt->execute();
		...
		if ($row['category'] == "50" && $user['rank'] < 6) {
			exit;
		}
		...
		if ($exists > 0) {
			...
			<?php
				...
				$stmt =  $bdd->prepare("UPDATE users SET credits = credits - :price WHERE id = :my_id LIMIT 1");
				...
				$stmt =  $bdd->prepare("INSERT INTO cms_transactions (userid, amount, date, descr) VALUES (:my_id, '-:price', :date_full, 'Webstore purchase')");
				$stmt->bindParam(':my_id', $my_id);
				$stmt->bindParam(':price', $row['price']);
				$stmt->bindParam(':date_full', $date_full);
				$stmt->execute();
...
<p>Watch this space - we add new items all the time!</p>
	...
	<div class=\"webstore-frank-image\"><img src=\"./web-gallery/images/frank/sorry.gif\" alt=\"\" width=\"57\" height=\"88\" /></div>
		...
		if ($exists > 0) {
			...
			?>
				...
				$stmt =  $bdd->prepare("SELECT id FROM cms_homes_inventory WHERE userid = :my_id AND type = '4' AND data = :data LIMIT 1");
				$stmt->bindParam(':my_id', $my_id);
		...
		if ($exists > 0) {
			...
			<?php
				...
				$stmt->bindValue(':my_id', $my_id);
				$stmt->bindValue(':price', $row['price']);
				$stmt->bindValue(':date_full', $date_full);
				$stmt->execute();
		...
		if ($_POST['type'] == "widgets") {
			...
			$productId = FilterText($_POST['itemId']);
			if (!is_numeric($productId)) {
				exit;
			}

			$stmt =  $bdd->prepare("SELECT * FROM cms_homes_inventory WHERE id = :productId AND userid = :my_id LIMIT 1");
			$stmt->bindParam(':productId', $productId);
			$stmt->bindParam(':my_id', $my_id);
			$stmt->execute();
		...
		header("X-JSON: [\"" . FormatThing($row['type'], $row['data'], true) . "\",\"" . FormatThing($row['type'], $row['data'], false) . "\",\"8\",\"" . $_POST['type'] . "\",null," . $amount . "]");
		...
		<div id="inventory-preview-place" class="clearfix">
			...
			<?php if (isset($row['amount']) && $row['amount'] > 1 && $row['type'] == "1") { ?>
				<div class="clearfix">
	...
	<div class=\"n_skin_" . $skin . "\" >
		...
		if (strlen($data) < 501 && strlen($data) > 0) {
			...
			if ($linked > 0) {
				$stmt =  $bdd->prepare("INSERT INTO cms_homes_stickers (userid, groupid, x, y, z, data, type, subtype, skin) VALUES (:my_id, :groupid, '10', '10', '18', :data, '3', '0', :skin)");
				$stmt->bindParam(':my_id', $my_id);
				$stmt->bindParam(':groupid', $groupid);
				$stmt->bindParam(':data', FilterText($data));
				$stmt->bindParam(':skin', $skin);
				$stmt->execute();

				$stmt =  $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :my_id AND groupid = :groupid AND type = '3' AND data = :data ORDER BY id DESC LIMIT 1");
				$stmt->bindParam(':my_id', $my_id);
				$stmt->bindParam(':groupid', $groupid);
				$stmt->bindParam(':data', FilterText($data));
				$stmt->execute();

				$stmt2 =  $bdd->prepare("SELECT id FROM cms_homes_inventory WHERE userid = :my_id AND type = '3' LIMIT 1");
				$stmt2->bindParam(':my_id', $my_id);
				$stmt2->execute();
			} else {
				$stmt =  $bdd->prepare("INSERT INTO cms_homes_stickers (userid, groupid, x, y, z, data, type, subtype, skin) VALUES (:my_id, '-1', '10', '10', '18', :data, '3', '0', :skin)");
				$stmt->bindParam(':my_id', $my_id);
				$stmt->bindParam(':data', FilterText($data));
				$stmt->bindParam(':skin', $skin);
				$stmt->execute();

				$stmt =  $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :my_id AND groupid = '-1' AND type = '3' AND data = :data ORDER BY id DESC LIMIT 1");
				$stmt->bindParam(':my_id', $my_id);
				$stmt->bindParam(':data', FilterText($data));
				$stmt->execute();
...
Event.observe(\"stickie-" . $id . "-edit\", \"click\", function(e) { openEditMenu(e, " . $id . ", \"stickie\", \"stickie-" . $id . "-edit\"); }, false);
	...
	<div class=\"n_skin_" . $skin . "\" >
		...
		$stmt =  $bdd->prepare("SELECT data, amount FROM cms_homes_inventory WHERE userid = :my_id AND type = '1' AND id = :id LIMIT 1");
		$stmt->bindParam(':my_id', $my_id);
		$stmt->bindParam(':id', $id);
		$stmt->execute();
		$exists = $stmt->rowCount();
		...
		if ($exists > 0) {
			...
			for ($i = 1; $i <= $amount; $i++) {
				if ($linked > 0) {
					$stmt =  $bdd->prepare("INSERT INTO cms_homes_stickers (userid, groupid, x, y, z, type, subtype, data, skin) VALUES (:my_id, :groupid, '10', '10', :z, '1', '0', :data, '')");
					$stmt->bindParam(':my_id', $my_id);
					$stmt->bindParam(':groupid', $groupid);
					$stmt->bindParam(':z', $z);
					$stmt->bindParam(':data', $row['data']);
					...
					$stmt =  $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :my_id AND groupid = :groupid AND type = '1' AND data = :data ORDER BY id DESC LIMIT 1");
					$stmt->bindParam(':my_id', $my_id);
					$stmt->bindParam(':groupid', $groupid);
					$stmt->bindParam(':data', $row['data']);
					$stmt->execute();
				} else {
					$stmt =  $bdd->prepare("INSERT INTO cms_homes_stickers (userid, groupid, x, y, z, type, subtype, data, skin) VALUES (:my_id, '-1', '10', '10', :z, '1', '0', :data, '')");
					$stmt->bindParam(':my_id', $my_id);
					$stmt->bindParam(':z', $z);
					$stmt->bindParam(':data', $row['data']);
					$stmt->execute();

					$stmt =  $bdd->prepare("SELECT id FROM cms_homes_stickers WHERE userid = :my_id AND groupid = '-1' AND type = '1' AND data = :data ORDER BY id DESC LIMIT 1");
					$stmt->bindParam(':my_id', $my_id);
					$stmt->bindParam(':data', $row['data']);
					$stmt->execute();
				}
...
Event.observe(\"sticker-" . $assoc['id'] . "-edit\", \"click\", function(e) { openEditMenu(e, " . $assoc['id'] . ", \"sticker\", \"sticker-" . $assoc['id'] . "-edit\"); }, false);
			...
			if ($placeAll == "true") {
				$stmt = $pdo->prepare("DELETE FROM cms_homes_inventory WHERE userid = :my_id AND id = :id AND type = '1' LIMIT 1");
				$stmt->bindParam(':my_id', $my_id);
				$stmt->bindParam(':id', $id);
				$stmt->execute();
			} else {
