...
if(isset($_GET['do'])) {
	$do = Secu($_GET['do']);
        ...
        <div class=\"redeem-error\"> 
			...
			$insertn2 = $bdd->prepare("INSERT INTO user_badges (user_id,badge_id,badge_slot) VALUES (:userid, :badge, :slot)");
...
<p class='credits-countries-select'><form name="achat_badge" action="?do=buy" method="post"><center><input type="image" src="<?PHP echo $imagepath;?>v2/images/valider.png"></center></p>
<center><ul id="credits-methods"><li id="credits-type-promo">
<tbody>
  <?php 
...
<script type="text/javascript">
    new PurseHabblet();
</script></div><script type="text/javascript">if (!$(document.body).hasClassName('process-template')) { Rounder.init(); }</script>             
...
Pour acheter un badge, il n'y a rien de plus simple! Il suffit d'appuyer sur le bouton rond dans la colonne "Action" puis d'appuyer sur "Valider" en haut de la page.</div>
