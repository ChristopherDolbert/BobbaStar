...
if ($_GET['do'] === "create" && isset($_POST['titre'])) {
	...
	if ($titre !== "") {
		...
		$insertn1->execute(array(':pseudo' => $user['username'], ':action' => 'a créé un événement avec lien <b>(' . addslashes($titre) . ')</b>', ':date' => FullDate('full')));
...
<form name='editor' method='post' action="?do=create">
	...
	<td width='80%' class='tbl'>HTTP://<input type="text" name="url" value="<?php if (!empty($_POST["url"])) {
