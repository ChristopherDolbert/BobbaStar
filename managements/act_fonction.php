...
$modifierrecrut = Secu($_GET['modifierrecrut']);
 $sql = $bdd->query("SELECT * FROM users WHERE id = '".$modifierrecrut."'");
 $a = $sql->fetch();
	...
	$assocaz = $sqlaz->fetch(PDO::FETCH_ASSOC);
      if($fonction != "") {
        $insertn1 = $bdd->prepare("INSERT INTO gabcms_stafflog (pseudo,action,date) VALUES (:pseudo, :action, :date)");
            ...
            $insertn1->bindValue(':action', 'a modifié la fonction de <b>'.$assocaz['username'].'</b> : <b>'.addslashes($fonction).'</b> (auparavant : '.addslashes($a['fonction']).')');
