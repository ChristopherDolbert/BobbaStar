    ...
    if(is_numeric($action)) {
        ...
        if(Secu($r['depuis']) <= Secu($nowtime) && Secu($r['jusqua']) > Secu($nowtime) && Secu($r['etat'] == 1)) {
            if($r['pseudo'] == $user['username']) {      
                ...
                $insertn1 = $bdd->prepare("INSERT INTO gabcms_stafflog (pseudo,action,date) VALUES (:pseudo, :action, :date)");
                    $insertn1->bindValue(':pseudo', $user['username']);
                    $insertn1->bindValue(':action', 'a annulé l\'absence de <b>'.$r['pseudo'].'</b>');
                    $insertn1->bindValue(':date', FullDate('full'));
                $insertn1->execute(); 
                $affichage = "<div id=\"purse-redeem-result\"> 
                    <div class=\"redeem-error\"> 
                        <div class=\"rounded rounded-green\"> 
                          Tu viens d'annuler l'absence de <b>".$r['pseudo']."</b> avec succès !
                        </div> 
                    </div> 
                </div>";
            } } else {
...
<div class="cbb clearfix orange"><h2 class="title">Infos</h2> 
...
<div class="habblet-container">		
<div class="cbb clearfix green"><h2 class="title">Remarques</h2> 
<div class="box-content">
...
?>
        <tr class="bas">
            ...
            ?><a href="?action=<?PHP echo Secu($a['id']); ?>" onclick="return confirm('Es-tu certains d\'annuler cette absence ?')" title="Annuler cette absence" onmouseover="tooltip.show(this)" onmouseout="tooltip.hide(this)"><img src="<?PHP echo $url; ?>/managements/img/images/invalide.gif" /></a><?PHP } } ?></td>
        </tr>
<?PHP } ?>
</tbody>
</table>
