...
if (isset($_POST['pseudo_recherche']) && !empty($_POST['pseudo_recherche'])) {
    ...
    $assoc = $sql->fetch(PDO::FETCH_ASSOC);
    if ($assoc['username'] != '') {
        $nb_inscrita = $bdd->query("SELECT COUNT(*) AS id FROM gabcms_avertissement WHERE user_id = '" . $assoc['id'] . "'")->fetchColumn();
        $nb_inscritz = $bdd->query("SELECT COUNT(*) AS id FROM gabcms_avertissement WHERE user_id = '" . $assoc['id'] . "' AND annuler = '1'")->fetchColumn();
        ...
        echo '<h4 class="alert_success">Le staff <b>' . $pseudo . '</b> a reçu un total de <b>' . $nb_inscrita . ' avertissement(s)</b> dont <b>' . $nb_inscritz . ' avertissement(s) annulé(s)</b></h4>';
    } else {
...
<body>
    ...
    Tu peux envoyer un ou des avertissement(s) à des staffs, pour différentes raisons, ça figurera dans leur dossier.<br /><br />
    ...
    <form name='editor' method='post' action="#">
        ...
        <td width='80%' class='tbl'><input type='text' name='raison' placeholder='Raison' class='text' style='width: 240px' maxlength='450'><br /></td>
        <td><b>Mon poste :</b> (pour afficher dans le dossier)<br /></td>
            ...
            <select name="poste" id="pays">
                ...
                if ($user['gender'] == 'M') {
                    ...
                    while ($a = $sql_a->fetch()) {
                        ...
                        $c = $correct->fetch();
                        echo '<option value="' . $a['poste'] . '">' . $c['nom_M'] . '</option>';
                    }
                }
                if ($user['gender'] == 'F') {
                    $sql_a = $bdd->query("SELECT * FROM gabcms_postes WHERE user_id = '" . $user['id'] . "' ORDER BY id DESC");
                    while ($a = $sql_a->fetch()) {
                        $correct = $bdd->query("SELECT * FROM gabcms_postes_noms WHERE id = '" . $a['poste'] . "'");
                        $c = $correct->fetch();
                        echo '<option value="' . $a['poste'] . '">' . $c['nom_F'] . '</option>';
                    }
                }
                ?>
            </select>
    ...
    <table>
        <tbody>
            ...
            <?php if (isset($_GET['annule']) || isset($_GET['do'])) { ?>
                ...
                while ($a = $sql->fetch()) {
                    echo '<tr class="bas">';
                    echo '<td class="bas">' . $assoca['username'] . '</td>';
                    echo '<td class="bas">' . $a['raison'] . '</td>';
                    echo '<td class="bas">' . $a['par'] . '</td>';
                    echo '<td class="bas">' . $a['date'] . '</td>';
                    if ($a['annuler'] == '0') {
                        echo '<td class="bas"><a href="?annule=' . $a['id'] . '" onclick="return confirm(\'Es-tu certain d\'annuler cet avertissement ? Cette action est irrévocable.\')">Annuler</a></td>';
                    }
                    echo '</tr>';
                }
                ?>
