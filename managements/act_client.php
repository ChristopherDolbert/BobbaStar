...
if (isset($_GET['do']) && isset($_POST['etat_client']) && $_GET['do'] == 'act_client') {
	$etat_client = Secu($_POST['etat_client']);
	$sql = $bdd->query("SELECT * FROM gabcms_config WHERE id = '1'");
	$cof = $sql->fetch(PDO::FETCH_ASSOC);

	if ($etat_client > 0 && $etat_client <= 3 && $etat_client != $cof['etat_client']) {
		$modif_etat = '';

		if ($etat_client == 1) {
			$modif_etat = '<span style="color:#008800;">Hôtel ouvert</span>';
		} elseif ($etat_client == 2 || $etat_client == 3) {
			$modif_etat = '<span style="color:#FF0000;">Hôtel fermé';

			if ($etat_client == 3) {
				$modif_etat .= ' (durée déterminée)';
			}
...
if (isset($_GET['do']) && $_GET['do'] == 'modif_etat_client') {
	...
	if ($dateDebutAbsence == "" || $dateFinAbsence == "" || $debuth == "" || $debutm == "" || $finh == "" || $finm == "") {
		echo '<h4 class="alert_error">Les dates et heures renseignées ne sont pas complètes.</h4>';
		exit();
	}

	$timestamp = mktime($debuth, $debutm, 01, explode("/", $dateDebutAbsence)[1], explode("/", $dateDebutAbsence)[0], explode("/", $dateDebutAbsence)[2]);
	$timestampz = mktime($finh, $finm, 00, explode("/", $dateFinAbsence)[1], explode("/", $dateFinAbsence)[0], explode("/", $dateFinAbsence)[2]);
...
<body>
	...
	<form name='editor' method='post' action="?do=act_client">
	...
	</form><?PHP if ($cof['etat_client'] == '3') { ?>
		<hr /><span class="HEADLINE">Fermeture de l'hôtel contrôlée</span><br />
		...
		<form name='editor' method='post' action="?do=modif_etat_client">
			...
			Heure & Minute : <select name="heuresf" id="lenght" class="select">
				...
				<option value="50">50</option>
				<option value="55">55</option>
			</select><br /><input type='submit' name='submit' value='Envoyer'>
		</form><?PHP } ?>
</body>
