...
if(isset($_GET['modifierrecrut'])) {
	...
	$r = $infr->fetch();
        ...
        $insertn1 = $bdd->prepare("INSERT INTO gabcms_stafflog (pseudo,action,date) VALUES (:pseudo, :action, :date)");
            ...
            if($assoc['gender'] == 'M') { $insertn1->bindValue(':action', '(tuteur) a donné son avis sur le test de <b>'.$assoc['username'].'</b> pour le poste de <b>'.addslashes($c['nom_M']).'</b>'); }
            elseif($assoc['gender'] == 'F') { $insertn1->bindValue(':action', '(tuteur) a donné son avis sur le test de <b>'.$assoc['username'].'</b> pour le poste de <b>'.addslashes($c['nom_F']).'</b>'); }
...
<span style="font-size:12px">En attente d&#39;avis depuis le <b><?PHP echo $date_but ?></b></span>
			...
			<td style="background-color: #D1D1D1; border-color:rgb(0, 0, 0); height:20px; text-align:left; vertical-align:top"><span style="font-size:12px">Avis du tuteur : <select name="avis" id="lenght" class="select"><option value="1">Favorable</option><option value="2">Défavorable</option></select></span></td>
