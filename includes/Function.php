...
if (!function_exists('IsUserBanned')) {
	...
	{
		include('SQL.php');
		$check_stmt = $bdd->prepare("SELECT * FROM bans WHERE user_id = :userid LIMIT 1");
		$check_stmt->execute(array(':userid' => $my_id));
		$is_banned = $check_stmt->rowCount();
		...
		if ($is_banned > 0) {
			$bandata = $check_stmt->fetch(PDO::FETCH_ASSOC);
			$reason = $bandata['ban_reason'];
			$expire = $bandata['ban_expire'];
			...
			if ($stamp_now < $stamp_expire) {
				...
				$delete_stmt = $con->prepare("DELETE FROM users_bans WHERE userid = :userid LIMIT 1");
				$delete_stmt->execute(array(':userid' => $my_id));
				return false;
			}
		} else {
			return false;
...
if (!function_exists('GetUserGroup')) {
	function GetUserGroup($my_id, $con)
	{
		$check_stmt = $con->prepare("SELECT groupid FROM groups_memberships WHERE userid = :userid AND is_current = '1' LIMIT 1");
		$check_stmt->execute(array(':userid' => $my_id));
		$has_fave = $check_stmt->rowCount();
...
if (!function_exists('FetchServerSetting')) {
...
if (!function_exists('FetchEmulatorSetting')) {
...
if (!function_exists('smileystaff')) {
...
if (!function_exists('smileyforum')) {
...
if (!function_exists('SecuriseText')) {
...
if (!function_exists('TicketRefresh')) {
...
if (!function_exists('Genere_code')) {
...
if (!function_exists('Genere_chiffre')) {
...
if (!function_exists('Genere_lettre')) {
...
if (!function_exists('ClientNitro')) {
...
if (!function_exists('GetUserGroupBadge')) {
