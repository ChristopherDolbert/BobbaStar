...
<?PHP   $userbadges = $bdd->query("SELECT DISTINCT * FROM user_badges WHERE user_id = '".$user['id']."'");
        while($userbadge = $userbadges->fetch())
        {
?>
        <tr class="bas">
            <td class="bas"><img src="<?PHP echo $swf_badge; echo $userbadge['badge_id']; ?>.gif" border="0" /></td>
            <td class="bas"><?php echo $userbadge['badge_id'];?></td>
        </tr>
<?PHP } ?>
    </tbody>
</table>
