...
if (isset($_POST['bean_avatarName']) || isset($_SESSION['bean_avatarName'])) {
    ...
    // Name validation
    if ($serverdb->num_rows($serverdb->query("SELECT id,name,email FROM users WHERE name = '" . $name . "' LIMIT 1")) > 0) {
    ...
    if ($failure == false) {
        ...
        $serverdb->query("INSERT INTO users (id,name,lastvisit,online,ipaddress_last,newsletter,email_verified,show_home,email_friendrequest,email_minimail,email,show_online) VALUES ('" . $row[0] . "','" . $row[1] . "','" . time() . "','" . time() . "','" . $_SERVER[REMOTE_ADDR] . "','" . $newsletter . "','0','1','1','1','" . $email . "','1')");
        if ($scredits > 0) {
            $db->query("INSERT INTO transactions (userid,time,amount,descr) VALUES ('" . $row[0] . "','" . time() . "','" . $scredits . "','Welcome to " . $sitename . "!')");
        }
        ...
        if ($settings->find("email_verify_enabled") == "1") {
            ...
            $hash = sha1($hash);
            $num = $key;
            $db->query("INSERT INTO verify (id,email,key_hash) VALUES ('" . $row[0] . "','" . $email . "','" . $hash . "')");
		...
		<p>
		' . $reward_text . '
		' . $lang->loc['email.verify.2'] . ' <a href="' . $url . '/email?key=' . $hash . '">' . $lang->loc['email.verify.2.b'] . '</a>
		</p>
		...
		<p>
		' . $lang->loc['email.verify.9'] . ' <a href="' . $url . '/email?remove=' . $hash . '">' . $lang->loc['email.verify.9.b'] . '</a>.
		</p>

		<p>
		' . $lang->loc['email.verify.11'] . '<a href="' . $url . '/help">' . $lang->loc['email.verify.12'] . '</a>.
		</p>';
            $mailer = new HoloMail;
            $mailer->sendSimpleMessage($to, $subject, $html);
        } else {
            $serverdb->query("UPDATE users SET email_verified = '1' WHERE id = '" . $row[0] . "' LIMIT 1");
        }

        // Referral
        if ($refer == true) {
            $data->update1($referrow[0], $settings->find("register_referral_rewards"));
            $db->query("INSERT INTO transactions (userid,time,amount,descr) VALUES ('" . $referrow[0] . "','" . time() . "','" . $settings->find("register_referral_rewards") . "','Referring a user.')");
        ...
        header("Location: " . $url . "/security_check?page=./welcome");
