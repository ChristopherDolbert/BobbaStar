...
if(isset($_POST['query'])){
	...
	$check = mysqli_query($con,("SELECT id,name FROM users WHERE name = '".$query."' LIMIT 1") or die(mysql_error());
	...
	if($results > 0 && is_numeric($amount)){
		...
		mysqli_query($con,("UPDATE users SET credits = credits + ".$amount." WHERE id = '".$query."' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("INSERT INTO cms_transactions (userid,date,amount,descr) VALUES ('".$query."','".$date_full."','".$amount."','Housekeeping transfer/refund')") or die(mysql_error());
