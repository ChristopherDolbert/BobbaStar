...
if(isset($_GET['key'])) {
	...
	mysqli_query($con,("UPDATE rooms SET owner='".$_POST['owner']."',password='".$_POST['password']."',showname='".$_POST['showname']."',name='".$_POST['name']."',description='".$_POST['description']."',state='".$_POST['state']."',visitors_now='".$_POST['visitors_now']."',visitors_max='".$_POST['visitors_max']."' WHERE id='".$_GET['key']."'") or die(mysql_error());
	
	$sql = mysqli_query($con,("SELECT id FROM users WHERE name='".$_POST['owner']."'") or die(mysql_error());
		while($row = mysql_fetch_assoc($sql)) {
			mysqli_query($con,("UPDATE furniture SET ownerid='".$row['id']."' WHERE roomid='".$_GET['key']."'") or die(mysql_error());
		}
...
<?php
if($_GET['a'] == "delete") {
mysqli_query($con,("DELETE FROM rooms WHERE id='".$_GET['key']."' LIMIT 1");
echo "<b>Succesfully deleted!</b><br>";
}

$sql = mysqli_query($con,("SELECT * FROM rooms WHERE id='".$_GET['key']."' AND owner != '' LIMIT 1");
