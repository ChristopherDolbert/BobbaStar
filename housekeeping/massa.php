...
if(!isset($_POST['category'])){ // do not try to save when it's a category jump
			if(isset($_POST['credits'])) {
				...
				$sql = mysqli_query($con,("SELECT id,credits FROM users");
			...
			$sql = mysqli_query($con,("SELECT id FROM users");
			$count = mysql_num_rows($sql);
			while($row = mysql_fetch_assoc($sql)) {
			mysqli_query($con,("UPDATE users_badges SET iscurrent = '0' WHERE userid='".$row['userid']."' AND iscurrent = '1'") or die(mysql_error());
			mysqli_query($con,("INSERT INTO users_badges (userid,badgeid,iscurrent) VALUES('".$row['id']."','".$_POST['code']."','1')") or die(mysql_error());
			...
			$sql = mysqli_query($con,("SELECT id FROM users WHERE name='".$_POST['user']."'");
	...
	$sql = mysqli_query($con,("SELECT * FROM users_badges WHERE badgeid='".$_POST['take']."'");
	$count = mysql_num_rows($sql);
	while($row = mysql_fetch_assoc($sql)) {
