...
if($_GET['cat'] != ""){
	...
	$page = "main";
	$id = $_GET['key'];
	mysqli_query($con,("DELETE FROM cms_faq WHERE catid = '".$id."'");
	mysqli_query($con,("DELETE FROM cms_faq WHERE id = '".$id."' LIMIT 1");
	...
	$id = $_POST['id'];
	if($id == ""){
		mysqli_query($con,("INSERT INTO cms_faq (type,title,content) VALUES ('cat','".$_POST['title']."','".FilterText($_POST['content'])."')");
	}else{
		mysqli_query($con,("UPDATE cms_faq SET title = '".$_POST['title']."', content = '".FilterText($_POST['content'])."' WHERE id = '".$id."' LIMIT 1");
	}
	$msg = "Category saved.";
	...
	$catid = $_POST['catid'];
	if($id == ""){
		mysqli_query($con,("INSERT INTO cms_faq (type,title,content,catid) VALUES ('item','".$_POST['topic']."','".FilterText($_POST['content'])."','".$catid."')");
	}else{
		mysqli_query($con,("UPDATE cms_faq SET title = '".$_POST['topic']."', content = '".FilterText($_POST['content'])."' WHERE id = '".$id."' LIMIT 1");
	}
	$msg = "Item saved.";
...
<?php
$sql = mysqli_query($con,("SELECT * FROM cms_faq WHERE type = 'cat' ORDER BY id ASC") or die(mysql_error());
...
<?php
if(isset($_GET['key'])){
	$row = mysql_fetch_assoc(mysqli_query($con,("SELECT * FROM cms_faq WHERE id = '".$_GET['key']."' LIMIT 1"));
	$title = $row['title'];
...
$id = $_GET['cat'];
$sql = mysqli_query($con,("SELECT * FROM cms_faq WHERE id = '".$id."'");
...
<?php
$sql = mysqli_query($con,("SELECT * FROM cms_faq WHERE catid = '".$id."'");
