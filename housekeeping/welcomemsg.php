...
if(isset($_POST['welcomemessage_enable'])){
	...
	if(!empty($welcomemessage_text)){
		...
		mysqli_query($con,("UPDATE system_config SET sval = '".$welcomemessage_enable."' WHERE skey = 'welcomemessage_enable' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_strings SET var_en = '".$welcomemessage_text."' WHERE stringid = 'welcomemessage_text' LIMIT 1") or die(mysql_error());

		$msg = "Settings saved successfully.";

		mysqli_query($con,("INSERT INTO system_stafflog (action,message,note,userid,targetid,timestamp) VALUES ('Housekeeping','Updated Server Settings (Welcome Message Options)','welcomemsg.php','".$my_id."','','".$date_full."')") or die(mysql_error());
...
<td class='tablerow2'  width='60%'  valign='middle'><textarea name='welcomemessage_text' cols='60' rows='5' wrap='soft' id='sub_desc'   class='multitext'><?php $check = mysqli_query($con,("SELECT var_en FROM system_strings WHERE stringid = 'welcomemessage_text' LIMIT 1") or die(mysql_error()); $row = mysql_fetch_assoc($check); echo $row['var_en']; ?></textarea></td>
</tr>
