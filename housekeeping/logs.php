...
<?php
$get_users = mysqli_query($con,("SELECT * FROM system_stafflog ORDER BY id DESC") or die(mysql_error());

while($row = mysql_fetch_assoc($get_users)){

	$userdata = mysqli_query($con,("SELECT name FROM users WHERE id = '".$row['userid']."' LIMIT 1");
	$userdata = mysql_fetch_assoc($userdata);

	if(!empty($row['targetid'])){
		$targetdata = mysqli_query($con,("SELECT name FROM users WHERE id = '".$row['targetid']."' LIMIT 1");
