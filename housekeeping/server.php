...
if(isset($_POST['game_port'])){
	...
	if(!empty($game_port) && !empty($mus_port) && !empty($game_maxconnections) && !empty($mus_maxconnections) && !empty($mus_host)){
		...
		mysqli_query($con,("UPDATE system_config SET sval = '".$game_port."' WHERE skey = 'server_game_port' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$mus_port."' WHERE skey = 'server_mus_port' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$game_maxconnections."' WHERE skey = 'server_game_maxconnections' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$mus_maxconnections."' WHERE skey = 'server_mus_maxconnections' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$mus_host."' WHERE skey = 'server_mus_host' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$trading_enable."' WHERE skey = 'trading_enable' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$chatanims_enable."' WHERE skey = 'chatanims_enable' LIMIT 1") or die(mysql_error());

		$msg = "Settings saved successfully.";

		mysqli_query($con,("INSERT INTO system_stafflog (action,message,note,userid,targetid,timestamp) VALUES ('Housekeeping','Updated Server Settings (General Configuration)','server.php','".$my_id."','','".$date_full."')") or die(mysql_error());
