...
if($do == "delete" && is_numeric($key)){
	...
	$check = mysqli_query($con,("SELECT num FROM cms_news WHERE num = '".$key."' LIMIT 1") or die(mysql_error());
	$exists = mysql_num_rows($check);

	if($exists > 0){
		mysqli_query($con,("DELETE FROM cms_news WHERE num = '".$key."' LIMIT 1") or die(mysql_error());
	...
	$check = mysqli_query($con,("SELECT * FROM cms_news WHERE num = '".$key."' LIMIT 1") or die(mysql_error());
	$exists = mysql_num_rows($check);
	...
	if($exists > 0){
		...
		mysqli_query($con,("UPDATE cms_news SET title = '".$title."', category = '".$category."', topstory = '".$topstory."', short_story = '".$short_story."', story = '".$story."' WHERE num = '".$num."' LIMIT 1") or die(mysql_error());
...
<?php
$get_articles = mysqli_query($con,("SELECT num,title,short_story,date,author FROM cms_news ORDER BY num DESC") or die(mysql_error());
