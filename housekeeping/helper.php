...
if($_GET['do'] == "pick" && isset($_GET['key']) && is_numeric($_GET['key'])){
	...
	$check = mysqli_query($con,("SELECT id FROM cms_help WHERE id = '".$_GET['key']."' AND picked_up = '0' LIMIT 1") or die(mysql_error());
	...
	$check = mysqli_query($con,("SELECT id FROM cms_help WHERE id = '".$_GET['key']."' AND picked_up = '1' LIMIT 1") or die(mysql_error());
	$found = mysql_num_rows($check);
...
<?php
$get_articles = mysqli_query($con,("SELECT id,username,ip,message,date,picked_up,subject,roomid FROM cms_help ORDER BY id DESC") or die(mysql_error());
