...
if(!isset($_POST['category']) || !isset($_GET['category'])){ // do not try to save when it's a category jump
    foreach($_POST as $key => $value){
        if(strlen($value) > 0 && strlen($key) > 0){
            mysqli_query($con,("UPDATE cms_content SET contentvalue = '".FilterText($value, true)."' WHERE contentkey = '".FilterText($key, true)."' LIMIT 1") or die(mysql_error());
...
<?php
    ...
    $get_settings = mysqli_query($con,("SELECT * FROM cms_content WHERE category = '".$catId."' ORDER BY contentkey ASC") or die(mysql_error());
