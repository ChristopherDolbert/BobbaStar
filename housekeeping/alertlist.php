...
<?php
$get_em = mysqli_query($con,("SELECT * FROM cms_alerts ORDER BY id DESC") or die(mysql_error());

while($row = mysql_fetch_assoc($get_em)){
	
	$check = mysqli_query($con,("SELECT name FROM users WHERE id = '".$row['userid']."' LIMIT 1") or die(mysql_error());
