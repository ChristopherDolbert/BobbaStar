...
$check = mysqli_query($con,("SELECT id FROM users WHERE name = '".$key."' OR id = '".$key."' LIMIT 1") or die(mysql_error());
		...
		if($rank > 0 && $rank < 8){
			if($rank > 6 && $sysadmin == $my_id || $rank < 7){
				if($sysadmin == $drow['id']){
					...
					mysqli_query($con,("DELETE FROM users_badges WHERE userid = '".$drow['id']."' AND badgeid = 'MOD' LIMIT 1");
					mysqli_query($con,("DELETE FROM users_badges WHERE userid = '".$drow['id']."' AND badgeid = 'ADM' LIMIT 1");
...
<?php
$get_vouchers = mysqli_query($con,("SELECT * FROM users WHERE rank > 2 ORDER BY name ASC LIMIT 50") or die(mysql_error());
?>
