...
if(isset($_POST['ip'])){
	...
	if(!empty($ip) && !empty($texts) && !empty($variables) && !empty($dcr) && !empty($reload_url)){
		...
		mysqli_query($con,("UPDATE cms_system SET ip = '".$ip."', texts = '".$texts."', variables = '".$variables."', dcr = '".$dcr."', reload_url = '".$reload_url."', localhost = '".$localhost."', loader='".$_POST['loader']."' LIMIT 1") or die(mysql_error());
		$msg = "Settings saved successfully.";
		mysqli_query($con,("UPDATE cms_content SET contentvalue = '".$_POST['widescreen']."' WHERE contentkey='client-widescreen'");

		mysqli_query($con,("INSERT INTO system_stafflog (action,message,note,userid,targetid,timestamp) VALUES ('Housekeeping','Updated CMS Settings (Loader Configuration)','loader.php','".$user['id']."','','".$date_full."')") or die(mysql_error());
...
<tr>
<td class='tablerow1'  width='40%'  valign='middle'><b>Adresse IP externe</b><div class='graytext'>The <b>external</b> IP address or hostname Holograph Emulator is located on. The second field is the port, wich is automaticly detected.</div></td>
...
<?php $wide = mysqli_query($con,("SELECT * FROM cms_content WHERE contentkey='client-widescreen'");
...
<?php $V23 = mysqli_query($con,("SELECT * FROM cms_system");
...
<tr>
<td class='tablerow1'  width='40%'  valign='middle'><b>Serveur est sur Localhost</b><div class='graytext'>If Holograph Emulator is running on the same server/computer as HoloCMS, set this to 'Yes'.</div></td>
