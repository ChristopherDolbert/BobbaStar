...
$check = mysqli_query($con,("SELECT id FROM users WHERE name = '".$key."' LIMIT 1") or die(mysql_error());
$exists = mysql_num_rows($check);
	...
	if($exists > 0){
		...
		if(strlen($badge) > 2 && strlen($badge) < 5){
			...
			$check = mysqli_query($con,("SELECT * FROM users_badges WHERE userid = '".$userid."' AND badgeid = '".$badge."' LIMIT 1") or die(mysql_error());
			$already_has_badge = mysql_num_rows($check);

			if($already_has_badge < 1){

				mysqli_query($con,("UPDATE users SET badge_status = '1' WHERE name = '".$key."' LIMIT 1") or die(mysql_error()); 
				mysqli_query($con,("INSERT INTO users_badges (userid,badgeid,iscurrent) VALUES ('".$userid."','".$badge."','1')") or die(mysql_error());
				...
				mysqli_query($con,("INSERT INTO system_stafflog (action,message,note,userid,targetid,timestamp) VALUES ('Housekeeping','Gave user ".$badge." badge','badgetool.php','".$my_id."','".$userid."','".$date_full."')") or die(mysql_error());
                                ...
                                mysqli_query($con,("UPDATE users_badges SET iscurrent = '0' WHERE userid = '" . $userid . "'");
				mysqli_query($con,("UPDATE users_badges SET iscurrent = '1' WHERE badgeid = '" . $badge . "' AND userid = '" . $userid . "' LIMIT 1") or die(mysql_error());
