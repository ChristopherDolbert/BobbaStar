...
if($_GET['a'] == "delete"){
mysqli_query($con,("DELETE FROM cms_recommended WHERE id = '".$_GET['key']."'");
...
mysqli_query($con,("UPDATE cms_recommended SET type='".$_POST['type']."',rec_id='".$_POST['rec_id']."' WHERE id='".$_GET['key']."' LIMIT 1") or die(mysql_error());
$msg = "Succesfully modified!";
}elseif(isset($_POST['add'])) {
mysqli_query($con,("INSERT INTO cms_recommended (type,rec_id) VALUES ('".$_POST['type']."','".$_POST['rec_id']."')") or die(mysql_error());
...
<?php }elseif($_GET['a'] == "edit") {
		if(isset($_GET['key'])) {
		$sql = mysqli_query($con,("SELECT * FROM cms_recommended WHERE id='".HoloText($_GET['key'])."' LIMIT 1");
