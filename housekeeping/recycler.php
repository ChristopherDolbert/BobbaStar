...
if(isset($_POST['recycler_enable'])){
	...
	if(!empty($recycler_minownertime) && !empty($recycler_session_length) && !empty($recycler_session_expirelength)){
		...
		mysqli_query($con,("UPDATE system_config SET sval = '".$recycler_enable."' WHERE skey = 'recycler_enable' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$recycler_minownertime."' WHERE skey = 'recycle_minownertime' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$recycler_session_length."' WHERE skey = 'recycler_session_length' LIMIT 1") or die(mysql_error());
		mysqli_query($con,("UPDATE system_config SET sval = '".$recycler_session_expirelength."' WHERE skey = 'recycler_session_expirelength' LIMIT 1") or die(mysql_error());

		$msg = "Settings saved successfully.";

		mysqli_query($con,("INSERT INTO system_stafflog (action,message,note,userid,targetid,timestamp) VALUES ('Housekeeping','Updated Server Settings (Recycler Options)','recycler.php','".$my_id."','','".$date_full."')") or die(mysql_error());
