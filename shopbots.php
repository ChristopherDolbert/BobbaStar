    ...
    if ($do == "1") {
        if ($user['jetons'] >= $cp['bots']) {
            if (isset($_POST['text']) && isset($_POST['missions']) && isset($_POST['roomid']) && ($_POST['nom'])) {
                ...
                $userrooms2 = $bdd->query("SELECT * FROM rooms WHERE id = '" . $room_id . "' AND owner_name = '" . $user['username'] . "'");
                if ($userrooms2->rowCount() == 0) {
                    ...
                    $userrooms = $bdd->query("SELECT * FROM rooms WHERE owner_name = '" . $user['username'] . "'");
                        ...
                        <div class=\"redeem-error\"> 
                            <div class=\"rounded rounded-red\"> 
                              Le nom de ton bot est trop long
                                        ...
                                        $jetons = $cp['bots'];
                                        $insertn1 = $bdd->prepare("INSERT INTO bots (user_id, name, room_id, motto, figure, freeroam, rot, chat_lines) VALUES (:userid, :name, :room, :motto, :look, :mode, :chat);");
                        ...
                        <div class="box-content">
                            <center>
                                <form action="?buy=1" method="post" id="profileForm">
                                    <div style="float:left;"><img src="<?php echo $avatarimage; ?><?php echo $user['look']; ?>&action=std&gesture=srp&direction=3&head_direction=2&size=1&img_format=gif" /></div>
                                    <input style="width:70%" placeholder="Nom" type="text" name="nom" maxlength="25" value="" id="avatarmotto" required />
                                    <br /><br />
                                    <input style="width:70%" placeholder="Mission" type="text" name="missions" maxlength="38" value="" id="avatarmotto" required />
                                    <br /><br />
                                    <textarea style="width:70%" placeholder="Phrase" name="text" maxlength="200" value="" id="avatarmotto" required></textarea>
                                    <br /><br />
                                    <?php
                                    $userrooms = $bdd->query("SELECT * FROM rooms WHERE owner_id = '" . $user['id'] . "'");
                                    if ($userrooms->rowCount() == 0) {
                                        echo '<strong>Tu n\'as pas d\'appart, vas vite t\'en créer un!</strong>';
                                    } else {
                                        echo "<SELECT style='width:70%' NAME='roomid' onChange='FocusObjet()'>";
                                        while ($userroom = $userrooms->fetch()) {
                                            echo "<OPTION VALUE='$userroom[0]'>$userroom[3]</OPTION>\n";
                                        }
                                        echo "</SELECT>";
                                    }
                                    ?>
                                    <br /><br />
                                    <input type="submit" name="bots" value="Créer mon bot" />
                                    <input type="reset" name="bots" value="Vider les champs" />
                                </form>
                                <font color="#6699FF">
                                    <h3><u><b>Prix: <?PHP echo $cp['bots'] ?> jetons</b></u></h3>
                                </font>
                            </center>
                        </div>
                    </div>
