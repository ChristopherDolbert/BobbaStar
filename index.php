				...
				if(isset($_POST['username']) && isset($_POST['password'])) {
						...
						$password = GabCMSHash($_POST['password']);
							if(empty($username) || empty($password)) {
										...
										$sql = $bdd->query("SELECT id,disabled FROM users WHERE username = '".$username."' AND password = '".$password."' LIMIT 1");
										$row = $sql->rowCount();
...
<link rel="stylesheet" href="<?PHP echo $imagepath; ?>index/css/index.css" type="text/css"/>
        <link rel='stylesheet' type='text/css' href='http://fonts.googleapis.com/css?family=Ubuntu:300,400,500,600,700,800,400italic,700italic|Ubuntu+Condensed'>
    </head>
    <body>
        <?PHP if(isset($erreur)) { echo "<div id='error'>".$erreur."</div>"; }?>
        <div id='header'>
            <div class='logo'>
            ...
            <form class='form' action='?do=se_connecter' method='post'>
                ...
                <div id='right'>
                    ...
                    <input id='password' type='password' name='password' value='' placeholder='' maxlength='20' autocomplete='off'>
                    <a href="<?PHP echo $url; ?>/oubliemotdepasse">Oublié ?</a>
                </div>
                <input type='submit' name='submit' id='submit' value='Connexion'>
            </form>
            <span>Rejoins-nous, il y a <?PHP $tmp = $bdd->query("SELECT users_online FROM server_status LIMIT 1");
				$tma = $tmp->fetch(PDO::FETCH_ASSOC);
				echo $tma['users_online']; ?> connectés</span>
            <div id='social'>
                <img src='<?PHP echo $imagepath; ?>index/img/fb.png' onclick='window.location.href="http://facebook.com/<?PHP echo $compte_facebook; ?>"'>
                <img src='<?PHP echo $imagepath; ?>index/img/twitter.png' onclick='window.location.href="https://twitter.com/<?PHP echo $compte_twitter; ?>"'>
            </div>
        </div>
        ...
        <div id='content'>
            ...
            <div id='btn' onclick='window.location.href="register"'>
                <p>Inscris toi</p> <p>gratuitement</p>
            </div>
            <div id='sepa'></div>
...
Habbo est une marque d&eacute;pos&eacute;e de Sulake Corporation. Tous droits r&eacute;serv&eacute;s &agrave; leur(s) propri&eacute;taire(s) respectif(s).<br />Nous ne sommes pas approuv&eacute;s, affili&eacute;s ou offertes par Sulake Corporation LTD.<br><br><u>&copy; GabCMS v<?PHP echo $version; ?> - Créer par l'équipe de GabCMS</u>
        </div>
    </body>
</html>