...
if (isset($_GET['referral'])) {
    $referral = $input->Secu($_GET['referral']);
    if ($serverdb->num_rows($bdd->select1($referral)) > 0) {
        $refer = true;
...
if (isset($_POST['bean_avatarName'])) {
    ...
    $password = $input->Secu($_POST['password']);
    $retypedpassword = $input->Secu($_POST['retypedPassword']);
    $day = $input->Secu($_POST['bean_day']);
    $month = $input->Secu($_POST['bean_month']);
    $year = $input->Secu($_POST['bean_year']);
    $email = $input->Secu($_POST['bean_email']);
    $retypedemail = $input->Secu($_POST['bean_retypedEmail']);
    ...
    $figure = $input->Secu($_POST['bean_figure']);
    $gender = $input->Secu($_POST['bean_gender']);
    $newsletter = $input->Secu($_POST['bean_marketing']);
    $referid = $input->Secu($_POST['referral']);
    if (isset($_POST['referral'])) {
        $referral = $input->Secu($_POST['referral']);
        if ($serverdb->num_rows($bdd->select1($referral)) > 0) {
            $refer = true;
            $referrow = $db->fetch_row($bdd->select1($referral));
        }
    ...
    $name = $input->Secu($_SESSION['bean_avatarName']);
    $password = $input->Secu($_SESSION['password']);
    $retypedpassword = $input->Secu($_SESSION['retypedPassword']);
    $day = $input->Secu($_SESSION['bean_day']);
    $month = $input->Secu($_SESSION['bean_month']);
    $year = $input->Secu($_SESSION['bean_year']);
    $email = $input->Secu($_SESSION['bean_email']);
    $retypedemail = $input->Secu($_SESSION['bean_retypedEmail']);
    $accept_tos = $_SESSION['bean_termsOfServiceSelection'];
    $figure = $input->Secu($_SESSION['bean_figure']);
    $gender = $input->Secu($_SESSION['bean_gender']);
    $newsletter = $input->Secu($_SESSION['bean_marketing']);
    $referid = $input->Secu($_SESSION['referral']);
    if (isset($_SESSION['referral'])) {
        $referral = $input->Secu($_SESSION['referral']);
        if ($serverdb->num_rows($bdd->select1($referral)) > 0) {
            $refer = true;
            $referrow = $db->fetch_row($bdd->select1($referral));
        }
...
if (isset($_POST['bean_avatarName']) || isset($_SESSION['bean_avatarName'])) {
    ...
    if ($failure == false) {
        ...
        $bdd->insert1($name, $password, $dob, $figure, $gender, $scredits);
        $row = $serverdb->fetch_row($bdd->select3($name));
        ...
        if ($settings->find("email_verify_enabled") == "1") {
            ...
            if ($settings->find("email_verify_reward") != "0") {
                $reward_text = $lang->loc['email.reward'] . " " . $settings->find("email_verify_reward") . " " . $lang->loc['credits'];
            } else {
                $reward_text = "";
            }
        ...
        // Referral
        if ($refer == true) {
            $bdd->update1($referrow[0], $settings->find("register_referral_rewards"));
                            ...
                            <div class="box-content">
                                ...
                                <fieldset>
                                    ...
                                    <div id="register-birthday"><select name="bean.day" id="bean_day" class="dateselector">
                                            <option value=""><?php echo $lang->loc['day']; ?></option>
                                            ...
                                            <option value=""><?php echo $lang->loc['month']; ?></option>
                                            <option value="1"><?php echo $months[0]; ?></option>
                                            <option value="2"><?php echo $months[1]; ?></option>
                                            <option value="3"><?php echo $months[2]; ?></option>
                                            <option value="4"><?php echo $months[3]; ?></option>
                                            <option value="5"><?php echo $months[4]; ?></option>
                                            <option value="6"><?php echo $months[5]; ?></option>
                                            <option value="7"><?php echo $months[6]; ?></option>
                                            <option value="8"><?php echo $months[7]; ?></option>
                                            <option value="9"><?php echo $months[8]; ?></option>
                                            <option value="10"><?php echo $months[9]; ?></option>
                                            <option value="11"><?php echo $months[10]; ?></option>
                                            <option value="12"><?php echo $months[11]; ?></option>
                                        </select> <select name="bean.year" id="bean_year" class="dateselector">
                                            <option value=""><?php echo $lang->loc['year']; ?></option>
                            ...
                            <div class="box-content">
                                ...
                                <fieldset id="register-fieldset-captcha">
                                    ...
                                    <div id="captcha-error-box">
                                        <div class="register-error">
                                            <div class="rounded rounded-red"><?php echo $lang->loc['error.1']; ?></div>
                                        </div>
                                    </div>
                                    <div class="register-label"><label for="register-captcha"><?php echo $lang->loc['type.in.code']; ?></label></div>
                                    ...
                                    <script type="text/javascript">
                                        document.observe("dom:loaded", function() {
                                            Event.observe($("captcha-reload"), "click", function(e) {
                                                Utils.reloadCaptcha()
                                            });
                                        });
                                    </script>
