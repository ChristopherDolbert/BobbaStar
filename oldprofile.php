...
if (isset($_GET['tag']) || isset($_GET['name']) || isset($_POST['name'])) {
    ...
    $user_sql = $bdd->prepare("SELECT * FROM users WHERE username = :searchname LIMIT 1");
    $user_sql->bindParam(':searchname', $searchname);
    $user_sql->execute();
    ...
    if ($user_exists == 1) {
        ...
        $pagename = "HomePage - " . $user_row['username'];
        if ($user_row['rank'] >= 6) {
            $drank = "Administrator";
    ...
    $stmt = $bdd->prepare("SELECT * FROM users WHERE id = :searchid LIMIT 1");
...
if (isset($_GET['do']) && $_GET['do'] == "edit" && $logged_in) {
    if ($user_row['username'] == $name) {
        $edit_mode = true;
        $stmt = $bdd->prepare("UPDATE cms_homes_group_linker SET active = '0' WHERE userid = :my_id LIMIT 1");
        $stmt->bindParam(':my_id', $my_id);
        $stmt->execute();
    } else {
        header("location:user_profile.php?do=bounce&name=" . $user_row['username']);
        exit;
        $edit_mode = false;
    }
} else {
...
if ($searchname == $name && $logged_in) {
    $pageid = "myprofile";
} else {
    $pageid = "profile";
}

$bg_stmt = $bdd->prepare("SELECT data FROM cms_homes_stickers WHERE type = '4' AND userid = :user_id AND groupid = '-1' LIMIT 1");
$bg_stmt->bindParam(':user_id', $user_row['id']);
$bg_stmt->execute();
$bg_exists = $bg_stmt->rowCount();


if ($bg_exists < 1) { // if there's no background override for this user set it to the standard
    $bg = "b_bg_pattern_abstract2";
} else {
    $bg_row = $bg_stmt->fetch(PDO::FETCH_NUM);
    $bg = "b_" . $bg_row[0];
}


if (!$error && !IsUserBanned($user_row['id'])) {
    include('template/hsubheader.php');
        ...
        <div id="content" style="position: relative" class="clearfix">
            <div id="mypage-wrapper" class="cbb blue">
                <div class="box-tabs-container box-tabs-left clearfix">
                    <?php if ($user_row['username'] == $name && $edit_mode !== true) { ?><a href="user_profile.php?do=edit&name=<?php echo FilterText($name); ?>" id="edit-button" class="new-button dark-button edit-icon" style="float:left"><b><span></span>Modifier</b><i></i></a><?php } ?>
                    ...
                    <?php if ($edit_mode == true) { ?>
                        <div id="top-toolbar" class="clearfix">
                            ...
                            <form action="#" method="get" style="width: 50%;">
                                <a id="cancel-button" class="new-button red-button cancel-icon" href="#"><b><span></span>Annuler</b><i></i></a>
                    ...
                    <div id="mypage-bg" class="<?php echo $bg; ?>">
                        <div id="playground-outer">
                            <div id="playground">
                                ...
                                <?php
                                $get_em_stmt = $bdd->prepare("SELECT id,type,x,y,z,data,skin,subtype,var FROM cms_homes_stickers WHERE userid = :user_id AND groupid = '-1' AND type < 4 LIMIT 200");
                                $get_em_stmt->bindParam(':user_id', $user_row['id']);
                                $get_em_stmt->execute();


                                while ($row = $get_em_stmt->fetch(PDO::FETCH_NUM)) {
			...
			<div class=\"stickie-content\">
				...
				<div class=\"stickie-footer\">
                                        ...
                                        if ($subtype == "GroupsWidget") {
                                            $groups_stmt = $bdd->prepare("SELECT COUNT(*) FROM guilds_members WHERE user_id = :user_id AND is_pending = '0' LIMIT 1");
                                            $groups_stmt->bindParam(':user_id', $user_row['id']);
                                            $groups_stmt->execute();
                                            $groups_result = $groups_stmt->fetchColumn();
...
<div class=\"w_skin_" . $row[6] . "\">
	<div class=\"widget-corner\" id=\"widget-" . $row[0] . "-handle\">
		<div class=\"widget-headline\"><h3><span class=\"header-left\">&nbsp;</span><span class=\"header-middle\">MY GROUPS (<span id=\"groups-list-size\">" . $groups_result . "</span>)</span><span class=\"header-right\">" . $edit . "</span></h3>
		</div>
	</div>
...
<ul class=\"groups-list\">";
                                            ...
                                            $get_groups_stmt = $bdd->prepare("SELECT * FROM guilds_members WHERE user_id = :user_id AND is_pending = '0'");
                                            $get_groups_stmt->bindValue(':user_id', $user_row['id'], PDO::PARAM_STR);
                                            $get_groups_stmt->execute();

                                            while ($membership_row = $get_groups_stmt->fetch(PDO::FETCH_ASSOC)) {
                                                $get_groupdata_stmt = $bdd->prepare("SELECT * FROM groups_details WHERE id = :group_id LIMIT 1");
                                                $get_groupdata_stmt->bindValue(':group_id', $membership_row['groupid'], PDO::PARAM_STR);
                                                $get_groupdata_stmt->execute();
        ...
        Group created:<br />";
                                                ...
                                                if ($membership_row['member_rank'] > 1 && $grouprow['ownerid'] !== $user_row['id']) {
                                                    echo "<div class=\"admin-group\" title=\"Admin\"></div>\n";
                                                }
                                                if ($grouprow['ownerid'] == $user_row['id'] && $membership_row['member_rank'] > 1) {
...
document.observe(\"dom:loaded\", function() {
	new GroupsWidget('" . $user_row['id'] . "', '" . $row[0] . "');
                                            ...
                                            $info_stmt = $bdd->prepare("SELECT * FROM users WHERE username = :searchname LIMIT 1");
                                            $info_stmt->bindParam(':searchname', $searchname);
                                            $info_stmt->execute();
                                            $userdata = $info_stmt->fetch(PDO::FETCH_ASSOC);
                                            $valid = $info_stmt->rowCount();
...
<span class=\"header-left\">&nbsp;</span><span class=\"header-middle\">MON " . strtoupper($shortname) . "</span><span class=\"header-right\">&nbsp;</span></h3>
		</div>
	</div>
	<div class=\"widget-body\">
		<div class=\"widget-content\">
	<div class=\"profile-info\">
		...
		<br class=\"clear\" />";
                                                ...
                                                if (IsUserOnline($userdata['id'])) {
                                                    echo "<img alt=\"online\" src=\"./web-gallery/images/myhabbo/habbo_online_anim_big.gif\" />";
                                                } else {
                                                    echo "<img alt=\"offline\" src=\"./web-gallery/images/myhabbo/habbo_offline_big.gif\" />";
                                                }
			...
			<div class=\"clear\"></div>
                                                ...
                                                if ($userdata['id'] != $my_id && $logged_in == true) { ?>
                                                    <div class="profile-friend-request clearfix">
                                                        <a href="./myhabbo/friends_add.php?id=<?php echo $userdata['id']; ?>" class="new-button" id="add-friend" style="float: left"><b>Envoyer un amigo</b><i></i></a>
                                                    </div>
                                            <?php }
...
<div id=\"profile-tags-container\">\n";
                                                ...
                                                $get_tags_stmt = $bdd->prepare("SELECT * FROM cms_tags WHERE ownerid = :owner_id ORDER BY id LIMIT 20");
                                                $get_tags_stmt->bindParam(':owner_id', $userdata['id'], PDO::PARAM_INT);
                                                $get_tags_stmt->execute();
                                                $rows = $get_tags_stmt->rowCount();

                                                $num = $rows;
                                                if ($num > 0) {
                                                    if ($userdata['id'] == $my_id && $logged_in) {
                                                        while ($row1 = $get_tags_stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            onMouseOut=\"this.src='./web-gallery/images/buttons/tags/tag_button_delete.gif'\"
            src=\"./web-gallery/images/buttons/tags/tag_button_delete.gif\"/>
                                                        ...
                                                        while ($row1 = $get_tags_stmt->fetch(PDO::FETCH_ASSOC)) {
                                                            printf("    <span class=\"tag-search-rowholder\">
                                                                <a href=\"tags.php?tag=%s\" class=\"tag-search-link tag-search-link-%s\">%s</a>
                                                                <img border=\"0\" class=\"tag-add-link tag-add-link-%s\"
                                                                onMouseOver=\"this.src='./web-gallery/images/buttons/tags/tag_button_add_hi.gif'\"
                                                                onMouseOut=\"this.src='./web-gallery/images/buttons/tags/tag_button_add.gif'\"
                                                                src=\"./web-gallery/images/buttons/tags/tag_button_add.gif\"/>
                                                                </span>", $row1['tag'], $row1['tag'], $row1['tag'], $row1['tag']);
                                                        }
                                                    } else {
                                                        while ($row1 = $get_tags_stmt->fetch(PDO::FETCH_ASSOC)) {
                                                            printf("    <span class=\"tag-search-rowholder\">
                                                                <a href=\"tags.php?tag=%s\" class=\"tag-search-link tag-search-link-%s\">%s</a></span>", $row1['tag'], $row1['tag'], $row1['tag']);
                                                        }
                                                    }
                                                } else {
                                                    echo "Pas de tags";
...
<div id=\"profile-tags-status-field\">
 ...
 <div class=\"content-red-bottom\">
  <div class=\"content-red-bottom-body\"></div>
            ...
            <a href=\"#\" class=\"new-button\" style=\"float:left;margin:5px 0 0 0;\" id=\"profile-add-tag\"><b>Ajouter un tag</b><i></i></a>
        </div>";
                                                }
                                                echo "</div>
    <script type=\"text/javascript\">
		...
		<div class=\"clear\"></div>
                                                    ...
                                                    <div class="widget-corner" id="widget-<?php echo $row['0']; ?>-handle">
                                                        <div class="widget-headline">
                                                            <h3>
                                                                ...
                                                                <span class="header-left">&nbsp;</span><span class="header-middle">MY ROOMS</span><span class="header-right">&nbsp;</span>
                                                            </h3>
                                                        </div>
                                                    </div>
                                                    <div class="widget-body">
                                                        <div class="widget-content">
                                                            <?php
                                                            $roomsql = $bdd->query("SELECT * FROM rooms WHERE owner_name = '" . $user_row['username'] . "'");
                                                            ...
                                                            ?>
                                                                ...
                                                                <div id="room_wrapper">
                                                                    <table border="0" cellpadding="0" cellspacing="0">
                                                                        ...
                                                                        <?php
                                                                        $i = 0;
                                                                        while ($roomrow = $roomsql->fetch(PDO::FETCH_ASSOC)) {
                                                                            $i++;
                                                                            if ($count == $i) {
                                                                                $asdf = " ";
                                                                            } else {
                                        ...
                                        <?php
                                            ...
                                            $sql = $bdd->query("SELECT * FROM cms_guestbook WHERE widget_id = '" . $row['0'] . "' ORDER BY id DESC");
                                            $count = $sql->rowCount();
                                                ...
                                                <div class="w_skin_<?php echo $row['6']; ?>">
                                                    <div class="widget-corner" id="widget-<?php echo $row['0']; ?>-handle">
                                                        <div class="widget-headline">
                                                            <h3>
                                                                <?php echo $edit; ?>
                                                                <span class="header-left">&nbsp;</span><span class="header-middle">My Guestbook(<span id="guestbook-size"><?php echo $count; ?></span>) <span id="guestbook-type" class="<?php echo $status; ?>"><img src="./web-gallery/images/groups/status_exclusive.gif" title="Friends only" alt="Friends only" /></span></span><span class="header-right">&nbsp;</span>
                                                            </h3>
                                                        </div>
                                                    </div>
                                                    <div class="widget-body">
                                                                ...
                                                                <ul class="guestbook-entries" id="guestbook-entry-container">
                                                                    ...
                                                                    <?php
                                                                        ...
                                                                        while ($row1 = $sql->fetch(PDO::FETCH_ASSOC)) {
                                                                            $i++;
                                                                            $userrow_stmt = $bdd->query("SELECT * FROM users WHERE id = '" . $row1['userid'] . "' LIMIT 1");
                                                                            $userrow = $userrow_stmt->fetch(PDO::FETCH_ASSOC);
                                            ...
                                            <div class="movable widget HighScoresWidget" id="widget-<?php echo $row['0']; ?>" style=" left: <?php echo $row['2']; ?>px; top: <?php echo $row['3']; ?>px; z-index: <?php echo $row['4']; ?>;">
                                                <div class="w_skin_<?php echo $row['6']; ?>">
                                                    ...
                                                    <div class="widget-body">
                                                        <div class="widget-content">
                                                            <?php
                                                            $bbsql_stmt = $bdd->query("SELECT * FROM users WHERE id = '" . $user_row['id'] . "' LIMIT 1");
                                                            $bbrow = $bbsql_stmt->fetch(PDO::FETCH_ASSOC);
                                        ...
                                        <?php
                                            ...
                                            $sql_stmt = $bdd->query("SELECT * FROM messenger_friendships WHERE userid = '" . $user_row['id'] . "' OR friendid = '" . $user_row['id'] . "'");
                                        ...
                                        ?>
                                            <div class="movable widget FriendsWidget" id="widget-<?php echo $row['0']; ?>" style=" left: <?php echo $row['2']; ?>px; top: <?php echo $row['3']; ?>px; z-index: <?php echo $row['4']; ?>;">
                                                <div class="w_skin_<?php echo $row['6']; ?>">
                                                    <div class="widget-corner" id="widget-<?php echo $row['0']; ?>-handle">
                                                        <div class="widget-headline">
                                                            <h3><?php echo $edit; ?><span class="header-left">&nbsp;</span><span class="header-middle">My Friends (<?php echo $count; ?>)</span><span class="header-right">&nbsp;</span></h3>
                                                        </div>
                                                    </div>
                                                    <div class="widget-body">
                                                        <div class="widget-content">
                                                ...
                                                <div class="w_skin_<?php echo $row['6']; ?>">
                                                    ...
                                                    <div class="widget-body">
                                                        <div class="widget-content">
                                                            ...
                                                            if ($edit_mode == true) { ?>
                                                                ...
                                                                <div id="edit-menu-trax-select-temp" style="display:none">
                                                                    <select id="trax-select-options-temp">
                                                                        ...
                                                                        <?php
                                                                        $mysql = $bdd->query("SELECT * FROM furniture WHERE ownerid = '" . $user_row['id'] . "'");
                                                                        $i = 0;
                                                                        while ($machinerow = $mysql->fetch(PDO::FETCH_ASSOC)) {
                                                                            $i++;
                                                                            $sql = $bdd->query("SELECT * FROM soundmachine_songs WHERE machineid = '" . $machinerow['id'] . "'");
                                                            ...
                                                            <?php } else {
                                                                $sql1_stmt = $bdd->query("SELECT * FROM soundmachine_songs WHERE id = '" . $row['8'] . "' LIMIT 1");
                                                                $songrow1 = $sql1_stmt->fetch(PDO::FETCH_ASSOC);
                                                            ?>
                                        ...
                                        <?php
                                            ...
                                            $sql_stmt = $bdd->query("SELECT * FROM users_badges WHERE user_id = '" . $user_row['id'] . "' ORDER BY badge_code ASC");
                                            $count = $sql_stmt->rowCount();
                                        ?>
                                            <div class="movable widget BadgesWidget" id="widget-<?php echo $row['0']; ?>" style=" left: <?php echo $row['2']; ?>px; top: <?php echo $row['3']; ?>px; z-index: <?php echo $row['4']; ?>;">
                                                <div class="w_skin_<?php echo $row['6']; ?>">
                                                    <div class="widget-corner" id="widget-<?php echo $row['0']; ?>-handle">
                                                        <div class="widget-headline">
                                                            <h3><?php echo $edit; ?><span class="header-left">&nbsp;</span><span class="header-middle">Badges</span><span class="header-right">&nbsp;</span></h3>
                                                        </div>
                                                    </div>
                                                    <div class="widget-body">
                                                        <div class="widget-content">
                                ...
                                if ($found_profile !== true) {
                                    ...
                                    $info_stmt = $bdd->query("SELECT * FROM users WHERE name = '" . $searchname . "' LIMIT 1");
                                    $userdata = $info_stmt->fetch(PDO::FETCH_ASSOC);
                                    $valid = $info_stmt->rowCount();
                                    ...
                                    if ($valid > 0) {
                                        ...
                                        $bdd->query("INSERT INTO cms_homes_stickers (userid,type,subtype,x,y,z,skin) VALUES ('" . $userdata['id'] . "','2','1','25','25','5','defaultskin')");
...
<span class=\"header-left\">&nbsp;</span><span class=\"header-middle\">MY " . strtoupper($shortname) . "</span><span class=\"header-right\">&nbsp;</span></h3>
	...
	<div class=\"profile-info\">
		...
		<br class=\"clear\" />";
                                        ...
                                        if (IsUserOnline($userdata['id'])) {
                                            echo "<img alt=\"online\" src=\"./web-gallery/images/myhabbo/habbo_online_anim.gif\" />";
                                        } else {
                                            echo "<img alt=\"offline\" src=\"./web-gallery/images/myhabbo/habbo_offline.gif\" />";
                                        }
...
<div id=\"profile-tags-container\">\n";
                                        ...
                                        $get_tags_stmt = $bdd->query("SELECT * FROM cms_tags WHERE ownerid = '" . $userdata['id'] . "' ORDER BY id LIMIT 20");
                                        $rows = $get_tags_stmt->rowCount();
                                        $num = $get_tags_stmt->rowCount();
                                        if ($num > 0) {

                                            if ($userdata['id'] == $my_id && $logged_in) {
                                                while ($row = $get_tags_stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		<div class=\"clear\"></div>
            ...
            <div id="footer">
                ...
                <p>Powered by HoloCMS &copy; 2008 Meth0d & Parts by Yifan, sisija.<br />
                    HABBO est une marque d&eacute;pos&eacute;e de Sulake Corporation LTD. Tous droits r&eacute;verv&eacute;s.<br />NakedCMS V2 est un CMS traduit et modifi&eacute; par l'&eacute;quipe de NakedGroup.</br> Merci de respecter leur travail et de ne pas copier, ou enlever ce copyright.<br />NakedCMS V2 by <a href="http://nakedgroup.o-n.fr">NakedGroup</a> 2008/2009</p>
