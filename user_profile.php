...
if (isset($_GET['tag']) || isset($_GET['name']) || isset($_POST['name'])) {
	...
	if (isset($_GET['tag'])) {
		$searchname = Secu($_GET['tag']);
	} else if (isset($_GET['name'])) {
		$searchname = Secu($_GET['name']);
	} else if (isset($_POST['name'])) {
		$searchname = Secu($_POST['name']);
	} else {
		$error = true;
	}


	$stmt = $bdd->prepare("SELECT * FROM users WHERE username = :searchname LIMIT 1");
	...
	if (isset($_GET['tagid'])) {
		$searchid = Secu($_GET['tagid']);
	} else if (isset($_GET['id'])) {
		$searchid = Secu($_GET['id']);
	} else if (isset($_POST['id'])) {
		$searchid = Secu($_POST['id']);
	} else {
		$error = true;
	}

	$stmt = $bdd->prepare("SELECT * FROM users WHERE id = :searchid LIMIT 1");
	$stmt->execute(array(':searchid' => $searchid));
	$user_row = $stmt->fetch(PDO::FETCH_ASSOC);
	$user_exists = $stmt->rowCount();
...
if (isset($_GET['do']) && $_GET['do'] == "edit" && $user['id']) {
	if ($user_row['username'] == $user['username']) {
		$edit_mode = true;
...
$bg_stmt->execute(array(':userid' => $user_row['id']));
		...
		<div id="content" style="position: relative" class="clearfix">
			<div id="mypage-wrapper" class="cbb blue">
				<div class="box-tabs-container box-tabs-left clearfix">
					<?php if ($user_row['username'] == $user['username'] && $edit_mode !== true) { ?><a href="user_profile.php?do=edit&name=<?php echo Secu($user['username']); ?>" id="edit-button" class="new-button dark-button edit-icon" style="float:left"><b><span></span>Modifier</b><i></i></a><?php } ?>
					...
					<div id="mypage-bg" class="<?php echo $bg; ?>">
						<div id="playground-outer">
							<div id="playground">
								...
								<?php
								$get_em_stmt = $bdd->prepare("SELECT id, type, x, y, z, data, skin, subtype, var FROM cms_homes_stickers WHERE userid = :userid AND groupid = '-1' AND type < 4 LIMIT 200");
								$get_em_stmt->execute(array(':userid' => $user_row['id']));
...
document.observe(\"dom:loaded\", function() {
	new GroupsWidget('" . $user_row['id'] . "', '" . $row[0] . "');
											...
											$info_stmt->execute(array(':searchname' => $searchname));
...
<span class=\"header-left\">&nbsp;</span><span class=\"header-middle\">MON " . strtoupper($shortname) . "</span><span class=\"header-right\">&nbsp;</span></h3>
	...
	<div class=\"profile-figure\">
			<img alt=\"" . $userdata['username'] . "\" src=\"" . $avatarimage . "" . $userdata['look'] . "&size=b&direction=4&head_direction=4&gesture=sml\" />
	</div>";
												if ($userdata['motto'] != null) {
													echo "<div class=\"profile-motto\">
			" . HoloText($userdata['motto']) . "
			<div class=\"clear\"></div>
		</div>";
												}
...
<div id=\"profile-tags-container\">\n";
												...
												$get_tags_stmt = $bdd->prepare("SELECT * FROM cms_tags WHERE ownerid = :ownerid ORDER BY id LIMIT 20");
												$get_tags_stmt->execute(array(':ownerid' => $userdata['id']));
												$rows = $get_tags_stmt->rowCount();
												$num = $rows;

												if ($num > 0) {
													while ($row1 = $get_tags_stmt->fetch(PDO::FETCH_ASSOC)) {
														if ($userdata['id'] == $user['id'] && $logged_in) {
    ...
    <script type=\"text/javascript\">
		document.observe(\"dom:loaded\", function() {
			new ProfileWidget('" . $userdata['id'] . "', '" . $userdata['id'] . "', {
				...
				messageText: \"Are you sure you want to ask <strong\>" . $userdata['username'] . "</strong\> to be your friend?\",
				buttonText: \"OK\",
															...
															?>
																...
																<div id="room_wrapper">
																	<table border="0" cellpadding="0" cellspacing="0">
																		...
																		while ($roomrow = $roomsql_stmt->fetch(PDO::FETCH_ASSOC)) {
																			$i++;
																			if ($count == $i) {
																				$asdf = " ";
																			} else {
...
<td $asdf>
		...
		<br class=\"clear\" />
										...
										<?php
											...
											$sql_stmt = $bdd->prepare("SELECT * FROM cms_guestbook WHERE widget_id = :widget_id ORDER BY id DESC");
											$sql_stmt->execute(array(':widget_id' => $row['0']));
														...
														<div class="widget-content">
															<div id="guestbook-wrapper" class="gb-public">
																<ul class="guestbook-entries" id="guestbook-entry-container">
																	...
																	<?php
																		$i = 0;
																		while ($row1 = $sql_stmt->fetch(PDO::FETCH_ASSOC)) {
																			$i++;
																			$user_stmt = $bdd->prepare("SELECT * FROM users WHERE id = :userid LIMIT 1");
																			$user_stmt->execute(array(':userid' => $row1['userid']));
																			$userrow = $user_stmt->fetch(PDO::FETCH_ASSOC);

																			if ($user['id'] == $row1['id'] || $user_row['id'] == $user['id']) {
		...
		<div class=\"guestbook-author\">
			<img src=\"" . $avatarimage . "figure=%s&direction=2&head_direction=2&gesture=sml&size=s\" alt=\"%s\" title=\"%s\"/>
		</div>
		...
		<div class=\"guestbook-entry-footer metadata\">%s</div>
	</li>", $row1['id'], $userrow['look'], $userrow['username'], $userrow['username'], $useronline, $userrow['id'], $userrow['username'], HoloText($row1['message'], false, true), $userrow['time']);
																		}
																	} ?>
																</ul>
															</div>
											...
											<div class="movable widget HighScoresWidget" id="widget-<?php echo $row['0']; ?>" style=" left: <?php echo $row['2']; ?>px; top: <?php echo $row['3']; ?>px; z-index: <?php echo $row['4']; ?>;">
												<div class="w_skin_<?php echo $row['6']; ?>">
													...
													<div class="widget-body">
														<div class="widget-content">
															<?php
															$bbsql_stmt = $bdd->prepare("SELECT * FROM users WHERE id = :userid LIMIT 1");
															$bbsql_stmt->execute(array(':userid' => $user_row['id']));
										...
										<?php
											...
											$sql_stmt = $bdd->prepare("SELECT * FROM messenger_friendships WHERE userid = :userid OR friendid = :userid");
											$sql_stmt->execute(array(':userid' => $user_row['id']));
											$count = $sql_stmt->rowCount();
												...
												<div class="w_skin_<?php echo $row['6']; ?>">
													...
													<div class="widget-body">
														<div class="widget-content">
															...
															if ($edit_mode == true) { ?>
																...
																<div id="edit-menu-trax-select-temp" style="display:none">
																	<select id="trax-select-options-temp">
																		...
																		<?php
																		$mysql_stmt = $bdd->prepare("SELECT * FROM furniture WHERE ownerid = :ownerid");
																		$mysql_stmt->execute(array(':ownerid' => $user_row['id']));
																		$i = 0;

																		while ($machinerow = $mysql_stmt->fetch(PDO::FETCH_ASSOC)) {
																			$i++;
																			$sql_stmt = $bdd->prepare("SELECT * FROM soundmachine_songs WHERE machineid = :machineid");
																			$sql_stmt->execute(array(':machineid' => $machinerow['id']));
																			$n = 0;

																			while ($songrow = $sql_stmt->fetch(PDO::FETCH_ASSOC)) {
																				$n++;
																				if ($songrow['id'] <> "") {
															...
															<?php } else {
																$sql1_stmt = $bdd->prepare("SELECT * FROM soundmachine_songs WHERE id = :song_id LIMIT 1");
																$sql1_stmt->execute(array(':song_id' => $row['8']));
										...
										<?php
											...
											$sql_stmt = $bdd->prepare("SELECT * FROM users_badges WHERE user_id = :userid ORDER BY badge_code ASC");
											$sql_stmt->execute(array(':userid' => $user_row['id']));
											$count = $sql_stmt->rowCount();
								...
								if ($found_profile !== true) {
									...
									if ($valid > 0) {
										...
										$insert_stmt = $bdd->prepare("INSERT INTO cms_homes_stickers (userid, type, subtype, x, y, z, skin) VALUES (:userid, '2', '1', '25', '25', '5', 'defaultskin')");
										$insert_stmt->execute(array(':userid' => $userdata['id']));
...
<span class=\"header-left\">&nbsp;</span><span class=\"header-middle\">MY " . strtoupper($shortname) . "</span><span class=\"header-right\">&nbsp;</span></h3>
	...
	<div class=\"profile-figure\">
			<img alt=\"" . $userdata['username'] . "\" src=\"" . $avatarimage . "figure=" . $userdata['look'] . "&size=b&direction=4&head_direction=4&gesture=sml\" />
	</div>
...
<div id=\"profile-tags-container\">\n";
            ...
            /></span>", $row['tag'], $row['tag'], $row['tag'], $row['tag']);
												...
												while ($row = $get_tags_stmt->fetch(PDO::FETCH_ASSOC)) {
													printf("    <span class=\"tag-search-rowholder\">
            <a href=\"tags.php?tag=%s\" class=\"tag-search-link tag-search-link-%s\"
            >%s</a></span>", $row['tag'], $row['tag'], $row['tag']);
												}
											}
										} else {
											echo "No tags";
