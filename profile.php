 ...
 $tab = Secu($_GET['tab']);
	if($tab == "general") {
				...
				$motto = Secu($_POST['motto']);
					...
					if($join!= "" && $online!= "" && $textamigo!= "" && $motto != "") {
					
				if(is_numeric($textamigo) && is_numeric($online) && is_numeric($join)) {
				if(!empty($_POST['envoimail'])) {
				$bdd->query("UPDATE users SET block_newfriends = '".$textamigo."', motto = '".$motto."', hide_online = '".$online."', hide_inroom = '".$join."', newsletter = '1' WHERE username = '".$user['username']."'");
			} else {
				$bdd->query("UPDATE users SET block_newfriends = '".$textamigo."', motto = '".$motto."', hide_online = '".$online."', hide_inroom = '".$join."', newsletter = '0' WHERE username = '".$user['username']."'");
			}
...
<p>
<label><input type="text" name="motto" value="<?PHP echo $user['motto']; ?>" class="text" style="width: 240px" maxlength="50"></label>
</p>
<h3>Textamigo</h3> 
<p> 
