...
<div id="container">
    ...
    <div id="column1" class="column">
				<div class="habblet-container ">
						...
						<?php 	
                        $sql = mysqli_query($con,("SELECT id,name FROM cms_application_forms WHERE enabled='1' AND deleted='0' ORDER BY name ASC");
						$sqll = mysqli_query($con,("SELECT id,name FROM cms_application_forms WHERE enabled='0' AND deleted='0' ORDER BY name ASC"); 						
                        ?>
Do YOU want to become a staff member? That's possible, below you see what jobs are free and what not. In total there are <b><?php echo mysql_num_rows($sql); ?></b> open applications and <b><?php echo mysql_num_rows($sqll); ?></b> closed applications</b>.<br><I>If there aren't any open applications, try again later.</I><br><br><b>Open applications:</b><br><?php
	...
	$sql = mysqli_query($con,("SELECT id,name FROM cms_application_forms WHERE enabled='0' AND deleted='0' ORDER BY name ASC");
...
<div id="container">
    ...
    <div id="column1" class="column">
				<div class="habblet-container ">
					...
					<?php
					$questionscheck = mysqli_query($con,("SELECT * FROM cms_application_questions WHERE aid='".$_GET['id']."'");
					if(mysql_num_rows($questionscheck) > 0) { ?>

					<br>

					<?php
					$sql = mysqli_query($con,("SELECT * FROM cms_application_questions WHERE aid='".$_GET['id']."' AND aoq='1'");
					if(mysql_num_rows($sql) > 0) { ?>
					<table cellspacing="1" cellpadding="0" width="420" border="3">
							<td><b>Questions</b><br><i>You're now getting some questions.</i><br>
							<?php
							while($row = mysql_fetch_assoc($sql)) {
							$questions = mysqli_query($con,("SELECT * FROM cms_application_questions WHERE qid='".$row['id']."'") or die(mysql_error()); ?>
